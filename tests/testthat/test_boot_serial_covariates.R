context("bootstrap test: serial multiple mediators, covariates")
...
boot_list <- list(
  robust = {
    ...
    test_mediation(test_data, x = x, y = y, m = m, covariates = covariates,
                   model = "serial", test = "boot", R = R, level = level[1],
...
for (method in methods) {
  ...
  test_that("dimensions are correct", {
    ...
    n_coef <- if (method %in% c("robust", "median", "OLS")) 15L else 19L
  ...
  test_that("values of coefficients are correct", {
    ...
    boot_a <- boot$reps$t[, c(2, 7)]
    boot_b <- boot$reps$t[, 11:12]
    boot_d <- boot$reps$t[, 6]
    boot_direct <- boot$reps$t[, 13]
    boot_indirect <- cbind(boot_a * boot_b, boot_a[, 1] * boot_d * boot_b[, 2])
  ...
  for (type in c("boot", "data")) {
    ...
    test_that("confint() method returns correct values of confidence intervals", {
      ...
      if (type == "boot") keep <- c("a", "b", "d", "total", "direct")
      else keep <- c("a", "b", "d", "direct")
    ...
    for (response in c(m, y)) {
      ...
      if (response == "Y") {
        ...
        coef_names <- c(intercept_name, m[1], x, covariates)
        n_coef <- 5L
        df1 <- 4
      } else {
  ...
  for (setting in c("wider", "less", "greater")) {
    ...
    test_that("retest() yields correct values", {
      ...
      if (setting == "wider") {
        ...
        if (setting == "less") {
          expect_equivalent(reboot$ci[, 1], rep.int(-Inf, 4))
          expect_equal(reboot$ci[, 2], boot$ci[, 2])
        } else {
          expect_equal(reboot$ci[, 1], boot$ci[, 1])
          expect_equivalent(reboot$ci[, 2], rep.int(Inf, 4))
        }
  ...
  for (setting in c("default", "p_value")) {
    ...
    test_that("object returned by setup_ci_plot() has correct structure", {
      ...
      names <- c("Direct", "Indirect_Total", "Indirect_M1",
                 "Indirect_M2", "Indirect_M1->M2")
  ...
  test_that("object returned by setup_density_plot() has correct structure", {
    ...
    # check dimensions
    expect_identical(dim(density$ci), c(4L, 4L))
    # check column names
