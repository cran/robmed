context("bootstrap test: parallel multiple mediators, no covariates")
...
for (method in methods) {
  ...
  test_that("dimensions are correct", {
    ...
    n_coef <- if (method %in% c("robust", "median", "OLS")) 8L else 10L
  ...
  test_that("values of coefficients are correct", {
    ...
    boot_a <- boot$reps$t[, c(2, 4)]
    boot_b <- boot$reps$t[, 6:7]
    boot_direct <- boot$reps$t[, 8]
    ...
    } else boot_total <- boot$reps$t[, 10]
  ...
  # tests for ellipse_plot(), which is only implemented for ROBMED and OLS
