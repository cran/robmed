...
for (method in methods) {
  ...
  test_that("dimensions are correct", {
    ...
    n_coef <- if (method %in% c("robust", "median", "OLS")) 9L else 11L
  ...
  test_that("values of coefficients are correct", {
    ...
    boot_a <- boot$reps$t[, c(2, 5)]
    boot_b <- boot$reps$t[, 7:8]
    boot_d <- boot$reps$t[, 4]
    boot_direct <- boot$reps$t[, 9]
    ...
    } else boot_total <- boot$reps$t[, 11]
